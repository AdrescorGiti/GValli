...
pub async fn remove_package(package: &str, noconfirm: bool) {
    ...
    if matches.is_empty() {
        eprintln!("❌ Пакет '{}' не найден в системе.", package);
        return;
    }

    // Выбор целевого пакета для удаления
    let target = if matches.len() == 1 || matches.iter().any(|m| m.2) {
        // Если нашли ровно 1 пакет или есть ТОЧНОЕ совпадение
        matches.iter().find(|m| m.2).unwrap_or(&matches[0]).clone()
    } else {
        // ИНТЕРАКТИВНЫЙ ВЫБОР: если совпадений несколько
